...
import { updateProceso, addEventoLog, updateDoc, getColaboradorByClave, addColaboradorToLog, getColaboradoresActivos, subscribeComentarios } from '@/lib/firebase-db';
...
export default function MonitoreoPage() {
        ...
        return (
            <div className="h-screen bg-background flex flex-col items-center justify-center gap-6">
                <div className="relative">
                    <div className="h-24 w-24 rounded-full border-4 border-primary-blue/20 border-t-primary-blue animate-spin" />
                    <Activity className="absolute inset-0 m-auto h-8 w-8 text-primary-blue animate-pulse" />
                </div>
                <div className="text-center">
                    <h2 className="text-2xl font-black text-white tracking-[0.2em]">CARGANDO PROCESO</h2>
                    <p className="text-gray-500 font-bold uppercase tracking-widest text-xs mt-2">Sincronizando con el servidor...</p>
                </div>
            </div>
        );
    }
    ...
    const trabajoCompletadoVal = proceso?.trabajoCompletado || 0;
    const cantidadProducirVal = proceso?.cantidadProducir || 1;
    const porcentaje = Math.min(100, Math.round((trabajoCompletadoVal / cantidadProducirVal) * 100) || 0);

    return (
        <div className="h-screen bg-background text-white flex flex-col overflow-hidden">
            {/* Header Compacto */}
            <header className="border-b border-white/10 bg-black/50 backdrop-blur-md p-2 flex items-center justify-between sticky top-0 z-50 shrink-0">
                <div className="flex flex-wrap items-center gap-3">
                    <button
                        onClick={handleBack}
                        className="p-1 px-2 hover:bg-white/10 rounded-lg transition-colors border border-white/5 bg-white/5"
                    >
            ...
            {/* Main Layout Grid */}
            <main className="flex-1 p-2 md:p-3 flex flex-col gap-3 md:gap-4 overflow-hidden max-w-[1900px] mx-auto w-full h-full">

                {/* Upper Section: Timer and Info Split */}
                <div className="flex-1 grid grid-cols-12 gap-3 md:gap-4 min-h-0">

                    {/* Left Column: Giant Timer */}
                    <div className={cn("flex flex-col gap-4 h-full min-h-0", proceso.utilizaTemporizador ? "col-span-12 lg:col-span-6" : "col-span-12")}>
                        ...
                        <div className="glass rounded-[3rem] p-4 md:p-6 flex flex-col items-center justify-center relative overflow-hidden group shadow-2xl border-white/5 flex-1 min-h-0">
                            ...
                            {/* CONTROL DE PERSONAL EXPRESS */}
                            <div className="mt-8 w-full pt-6 border-t border-white/5 max-w-4xl">
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-3">
                                        <Users className="h-4 w-4 text-success-green" />
                                        <h3 className="text-[clamp(0.6rem,0.9vw,0.75rem)] font-black tracking-[0.2em] uppercase text-gray-400">PERSONAL EN LÍNEA</h3>
                                    </div>
                                    <button
                                        onClick={() => setShowStaffModal(true)}
                                        className="px-3 py-1 bg-success-green/10 text-success-green rounded-full text-[clamp(0.5rem,0.7vw,0.6rem)] font-black border border-success-green/20 hover:bg-success-green/20 transition-all uppercase tracking-widest"
                                    >
                        ...
                        {['supervisor', 'superadmin'].includes(user?.rol || '') && proceso!.estado !== 'Finalizado' && getTipoProcesoReal(proceso!) === 'empaque' && (
                            <div className="mt-4 pt-4 border-t border-white/5">
                                <div className="flex items-center gap-2 mb-2">
                                    <Plus className="h-4 w-4 text-accent-purple" />
                                    <h3 className="text-[clamp(0.5rem,0.9vw,0.75rem)] font-black tracking-[0.2em] uppercase text-gray-400">Ajuste Manual</h3>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                    {[-10, -5, -1, 1, 5, 10, 50, 100].map(val => (
                                        <button
                                            ...
                                            className={cn(
                                                "px-3 py-1.5 rounded-lg font-black text-[10px] transition-all border",
                                                val > 0
                    ...
                    </div> {/* Closes the left column div */}

                    {/* Right Column: Metrics & Production Data - Solo si usa temporizador */}
                    {proceso!.utilizaTemporizador && (
                        <div className="col-span-12 lg:col-span-6 flex flex-col gap-4 overflow-auto pr-1 h-full min-h-0">
                            {/* PRODUCTION DATA (SIDEBAR) */}
                            <div className="glass rounded-[2.5rem] p-8 bg-gradient-to-br from-white/[0.05] to-transparent border-white/5 h-full flex flex-col">
                                <div className="space-y-4 flex-1">
                                    ...
                                    <div className="space-y-1 bg-primary-blue/10 p-4 rounded-3xl border border-primary-blue/20">
                                        <div className="flex items-center gap-2">
                                            <Users className="h-4 w-4 text-primary-blue" />
                                            <p className="text-[clamp(0.5rem,0.9vw,0.7rem)] font-black text-primary-blue uppercase tracking-widest">Líder de Línea</p>
                                        </div>
                                        <h4 className="text-[clamp(1.2rem,1.8vw,1.5rem)] font-black text-white uppercase">{proceso.lider}</h4>
                                    </div>

                                    {/* Orden de Producción - Más compacta */}
                                    <div className="space-y-1 bg-white/5 p-4 rounded-3xl border border-white/10">
                                        <p className="text-[clamp(0.5rem,0.9vw,0.7rem)] font-black text-gray-500 uppercase tracking-widest">Orden de Producción</p>
                                        <p className="text-[clamp(1.8rem,3vw,2.5rem)] font-mono font-black text-white leading-none">{proceso.ordenProduccion}</p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-3">
                                        <div className="bg-white/5 p-4 rounded-3xl border border-white/10 text-center">
                                            <p className="text-[clamp(0.5rem,0.9vw,0.7rem)] font-black text-gray-500 uppercase">Lote</p>
                                            <p className="text-[clamp(1.2rem,1.8vw,1.6rem)] font-black text-white uppercase truncate">{proceso.lote}</p>
                                        </div>
                                        <div className="bg-white/5 p-4 rounded-3xl border border-white/10 text-center">
                                            <p className="text-[clamp(0.5rem,0.9vw,0.7rem)] font-black text-gray-500 uppercase">Etapa</p>
                                            <p className="text-[clamp(1.2rem,1.8vw,1.6rem)] font-black text-white uppercase truncate">{proceso.etapa}</p>
                                        </div>
                                        <div className="bg-white/5 p-4 rounded-3xl border border-white/10 text-center">
                                            <p className="text-[clamp(0.5rem,0.9vw,0.7rem)] font-black text-gray-500 uppercase">Línea</p>
                                            <p className="text-[clamp(1.2rem,1.8vw,1.6rem)] font-black text-white uppercase truncate">{proceso.linea || 'N/A'}</p>
                                        </div>
                                    </div>

                                    {/* Fechas de Fabricación y Vencimiento */}
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-success-green/5 p-4 rounded-3xl border border-success-green/20 text-center flex flex-col items-center justify-center">
                                            <p className="text-[clamp(0.5rem,0.9vw,0.7rem)] font-black text-success-green/60 uppercase">Fabricación</p>
                                            <p className="text-[clamp(1.2rem,1.8vw,2rem)] font-mono font-black text-white">{proceso.fechaFabricacion || '--/--'}</p>
                                        </div>
                                        <div className="bg-danger-red/5 p-4 rounded-3xl border border-danger-red/20 text-center flex flex-col items-center justify-center">
                                            <p className="text-[clamp(0.5rem,0.9vw,0.7rem)] font-black text-danger-red/60 uppercase">Vencimiento</p>
                                            <p className="text-[clamp(1.2rem,1.8vw,2rem)] font-mono font-black text-white">{proceso.fechaExpira || '--/--'}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                {/* BOTTOM METRICS SECTION - Nueva distribución horizontal optimizada */}
                {proceso!.utilizaTemporizador && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5 mt-auto shrink-0">
                        <div className="glass p-4 md:p-5 rounded-[2.5rem] flex items-center justify-center gap-6 border-l-[10px] border-primary-blue shadow-lg bg-gradient-to-r from-primary-blue/5 to-transparent">
                            <div className="p-3 bg-primary-blue/10 rounded-2xl">
                                <Activity className="h-8 w-8 md:h-10 md:w-10 text-primary-blue" />
                            </div>
                            <div className="text-center md:text-left">
                                <p className="text-[clamp(0.6rem,1vw,0.8rem)] font-black text-gray-400 uppercase tracking-[0.3em] mb-1">Velocidad del Equipo</p>
                                <h3 className="text-[clamp(1.8rem,3vw,2.5rem)] font-black tracking-tight text-white leading-none">
                                    {stats.velocidadActual.toFixed(2)}
                                    <span className="text-sm text-gray-500 ml-4 font-bold">est/min</span>
                                </h3>
                            </div>
                        </div>

                        <div className="glass p-4 md:p-5 rounded-[2.5rem] flex items-center justify-center gap-6 border-l-[10px] border-accent-purple shadow-lg bg-gradient-to-r from-accent-purple/5 to-transparent">
                            <div className="p-3 bg-accent-purple/10 rounded-2xl">
                                <ClipboardList className="h-8 w-8 md:h-10 md:w-10 text-accent-purple" />
                            </div>
                            <div className="text-center md:text-left">
