...
export default function MonitoreoPage() {
    ...
    return (
        <div className="h-screen bg-background text-white flex flex-col overflow-hidden">
            ...
            {/* Main Layout Grid */}
            <main className="flex-1 p-4 md:p-6 flex flex-col gap-4 md:gap-6 overflow-hidden max-w-[1900px] mx-auto w-full h-full">

                {/* Upper Section: Timer and Info Split */}
                <div className="flex-1 grid grid-cols-12 gap-4 md:gap-6 min-h-0">

                    {/* Left Column: Giant Timer */}
                    <div className={cn("flex flex-col gap-6 h-full min-h-0", proceso.utilizaTemporizador ? "col-span-12 lg:col-span-6" : "col-span-12")}>
                    ...
                    {proceso!.utilizaTemporizador && (
                        <div className="col-span-12 lg:col-span-6 flex flex-col gap-6 overflow-auto pr-1 h-full min-h-0">
                            ...
                            <div className="glass rounded-[2.5rem] p-8 bg-gradient-to-br from-white/[0.05] to-transparent border-white/5 h-full flex flex-col">
                                <div className="space-y-8 flex-1">
                                    ...
                                    <div className="space-y-2">
                                        <p className="text-[clamp(0.7rem,1.2vw,0.9rem)] font-black text-gray-400 uppercase tracking-[0.3em]">Producto en proceso</p>
                                    ...
                                    <div className="grid grid-cols-3 gap-4">
                                        <div className="bg-white/5 p-5 rounded-3xl border border-white/10 text-center">
                                            ...
                                            <p className="text-[clamp(1.5rem,2.2vw,2rem)] font-black text-white uppercase truncate">{proceso.lote}</p>
                                        </div>
                                        <div className="bg-white/5 p-5 rounded-3xl border border-white/10 text-center">
                                            <p className="text-[clamp(0.6rem,1vw,0.75rem)] font-black text-gray-500 uppercase mb-1">Etapa</p>
                                            <p className="text-[clamp(1.5rem,2.2vw,2rem)] font-black text-white uppercase truncate">{proceso.etapa}</p>
                                        </div>
                                        <div className="bg-white/5 p-5 rounded-3xl border border-white/10 text-center">
                                            <p className="text-[clamp(0.6rem,1vw,0.75rem)] font-black text-gray-500 uppercase mb-1">Línea</p>
                                            <p className="text-[clamp(1.5rem,2.2vw,2rem)] font-black text-white uppercase truncate">{proceso.linea || 'N/A'}</p>
                                        </div>
                                    </div>
                ...
                {/* BOTTOM METRICS SECTION - Nueva distribución horizontal */}
                {proceso!.utilizaTemporizador && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mt-2 shrink-0">
                        <div className="glass p-5 md:p-8 rounded-[2.5rem] flex items-center justify-center gap-6 border-l-[10px] border-primary-blue shadow-xl bg-gradient-to-r from-primary-blue/5 to-transparent">
                            <div className="p-4 bg-primary-blue/10 rounded-3xl">
                                <Activity className="h-10 w-10 md:h-12 md:w-12 text-primary-blue" />
                            </div>
                            <div className="text-center md:text-left">
                                <p className="text-[clamp(0.7rem,1.2vw,0.9rem)] font-black text-gray-400 uppercase tracking-[0.4em] mb-1">Velocidad del Equipo</p>
                                <h3 className="text-[clamp(2.5rem,4vw,3.5rem)] font-black tracking-tight text-white leading-none">
                                    {stats.velocidadActual.toFixed(2)}
                                    <span className="text-xl text-gray-500 ml-4 font-bold">est/min</span>
                                </h3>
                            </div>
                        </div>

                        <div className="glass p-5 md:p-8 rounded-[2.5rem] flex items-center justify-center gap-6 border-l-[10px] border-accent-purple shadow-xl bg-gradient-to-r from-accent-purple/5 to-transparent">
                            <div className="p-4 bg-accent-purple/10 rounded-3xl">
                                <ClipboardList className="h-10 w-10 md:h-12 md:w-12 text-accent-purple" />
                            </div>
                            <div className="text-center md:text-left">
                                <p className="text-[clamp(0.7rem,1.2vw,0.9rem)] font-black text-gray-400 uppercase tracking-[0.4em] mb-1">Unidades Pendientes</p>
                                <h3 className="text-[clamp(2.5rem,4vw,3.5rem)] font-black tracking-tight text-white leading-none">
                                    {Math.max(0, proceso!.cantidadProducir - (proceso!.trabajoCompletado || 0)).toLocaleString()}
                                    <span className="text-xl text-gray-500 ml-4 font-bold uppercase">Uds</span>
                                </h3>
                            </div>
                        </div>
                    </div>
